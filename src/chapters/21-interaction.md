...
$$PD\_{jk}(\mathbf{x}\_j, \mathbf{x}\_k) = PD\_j(\mathbf{x}\_j) + PD\_k(\mathbf{x}\_k)$$

که در آن $PD\_{jk}(\mathbf{x}\_j, \mathbf{x}\_k)$ تابع وابستگی جزئی دوسویهٔ هر دو ویژگی است، و $PD\_j(\mathbf{x}\_j)$ و $PD\_k(\mathbf{x}\_k)$ توابع وابستگی جزئی تک‌تک ویژگی‌ها هستند.
...
$$H^{*}\_{jk} = \sqrt{\sum\_{i=1}^n\left[PD\_{jk}(x\_{j}^{(i)},x\_k^{(i)})-PD\_j(x\_j^{(i)}) - PD\_k(x\_{k}^{(i)})\right]^2}$$
