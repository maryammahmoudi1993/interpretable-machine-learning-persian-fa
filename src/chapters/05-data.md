...
> 
> $$I(X;Y) = \sum\_{i \in \mathcal{X}} \sum\_{j \in \mathcal{Y}} p(x\_i, y\_j) \log \frac{p(x\_i, y\_j)}{p(x\_i)p(y\_j)}$$
> 
> که در آن $i \in \mathcal{X}$ و $j \in \mathcal{Y}$. $p(x\_i)$ احتمال اینکه ویژگی $X$ دسته $i$ را بگیرد است، و برای $Y$ نیز همین‌طور. $p(x\_i, y\_j)$ احتمال توأم اینکه ویژگی $X$ دسته $i$ و $Y$ دسته $j$ را بگیرند است.
> 
...
> که در آن
> 
> $$H(X) = -\sum\_{i \in \mathcal{X}} p(x\_i) \log p(x\_i)$$
> 
> که در آن $\mathcal{X}$ مجموعه تمام دسته‌هایی است که $X$ می‌تواند بگیرد. برای استفاده از اطلاعات متقابل با ویژگی‌های عددی، مقادیر مشاهده‌شده $x\_1, \ldots, x\_n$ ویژگی $X$ را به بازه‌هایی با اندازه برابر گسسته‌سازی می‌کنیم. تعداد بازه‌ها با استفاده از قاعده Freedman-Diaconis تعیین می‌شود (Freedman و Diaconis 1981):
> 
