...
$$\text{MMD}^2 = \frac{1}{m^2} \sum\_{i,j=1}^m k(\mathbf{z}\_i, \mathbf{z}\_j) - \frac{2}{mn} \sum\_{i=1}^m \sum\_{j=1}^n k(\mathbf{z}\_i, \mathbf{x}\_j) + \frac{1}{n^2} \sum\_{i,j=1}^n k(\mathbf{x}\_i, \mathbf{x}\_j)$$
...
$$\mathrm{witness}(\mathbf{x})=\frac{1}{n}\sum\_{i=1}^{n}k(\mathbf{x}, \mathbf{x}^{(i)})-\frac{1}{m}\sum\_{j=1}^{m}k(\mathbf{x}, \mathbf{z}^{(j)})$$
...
$$\hat{f}(\mathbf{x})=\arg\max\_{i\in S}k(\mathbf{x},\mathbf{x}\_i)$$
