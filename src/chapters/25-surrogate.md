...
$$g(\mathbf{x}) = \beta\_0 + \beta\_1 x\_1 + \ldots + \beta\_p x\_p$$

یا یک درخت تصمیم:

$$g(\mathbf{x}) = \sum\_{m=1}^M c\_m I\{\mathbf{x} \in R\_m\}$$
...
$$R^2=1 - \frac{SSE}{SST} = 1 - \frac{\sum\_{i=1}^n (\hat{y}\_*^{(i)} - \hat{y}^{(i)})^2}{\sum\_{i=1}^n (\hat{y}^{(i)} - \bar{\hat{y}})^2}$$
