...
$$\text{explanation}(\mathbf{x}) = \arg\min\_{g \,\in\, \mathcal{G}} \;\mathcal{L}\\\!\left(\hat{f},\, g,\, \pi\_{\mathbf{x}}\right) + \Omega(g)$$
...
- $\mathcal{L}(\hat{f}, g, \pi\_{\mathbf{x}})$: تابع خطا که اندازه می‌گیرد $g$ تا چه حد پیش‌بینی‌های $\hat{f}$ را در همسایگی $\mathbf{x}$ (با وزن‌دهی $\pi\_{\mathbf{x}}$) تقریب می‌زند.
- $\Omega(g)$: پیچیدگی مدل $g$ (مثلاً تعداد ویژگی‌های غیرصفر در رگرسیون خطی).
- $\pi\_{\mathbf{x}}$: تابع مجاورت (kernel) که نزدیکی نمونه‌های اختلال‌یافته به $\mathbf{x}$ را اندازه می‌گیرد.
