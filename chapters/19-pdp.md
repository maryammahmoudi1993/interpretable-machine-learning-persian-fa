...
$$\hat{f}\_S(\mathbf{x}\_S) = \frac{1}{n} \sum\_{i=1}^{n} \hat{f}(\mathbf{x}\_S, \mathbf{x}\_C^{(i)})$$
...
$$I(\mathbf{x}\_S) = \sqrt{\frac{1}{K-1} \sum\_{k=1}^{K} \left(\hat{f}\_S(\mathbf{x}\_S^{(k)}) - \frac{1}{K}\sum\_{k=1}^{K} \hat{f}\_S(\mathbf{x}\_S^{(k)})\right)^2}$$

توجه داشته باشید که در اینجا $\mathbf{x}\_S^{(1)}, \ldots, \mathbf{x}\_S^{(K)}$ مقادیر $K$ منحصربه‌فرد ویژگی $X\_S$ هستند. برای ویژگی‌های طبقه‌ای داریم:

$$I(X\_S) = \frac{\max\_k(\hat{f}\_S(\mathbf{x}\_S^{(k)})) - \min\_k(\hat{f}\_S(\mathbf{x}\_S^{(k)}))}{4}$$
